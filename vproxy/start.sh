...
echo "VVproxy" > $allproxy_path/log/vproxy
...
then
	...
	openvpn --config $vproxy_path --auth-user-pass $(pwd)$vpnbook_cred_path --http-proxy-timeout 5 \
		--http-proxy $vproxy_server $vproxy_port $(pwd)$vproxy_cred_path basic > $allproxy_path/log/openvpn.log 2>&1 & 
	sleep 0.5
	echo `ps aux | grep -F 'openvpn' | grep -v -F 'grep' | awk '{ print $2 }'` > $allproxy_path/pid/vproxy
